vegStruct <- function(db, grpBy = NULL, polys = NULL, returnSpatial = FALSE, 
  ...
  out <- lapply(X = iter, FUN = vegStructStarter, db, 
                ...
                landType, method, lambda, areaDomain, byPlot, 
  ...
  if (!byPlot) {
    ...
    tEst <- tEst %>%
      ...
      dplyr::mutate(COVER_AREA_TOTAL = cover_mean,
                    AREA_TOTAL = fa_mean,
                    # Ratios
                    COVER_PCT = cover_mean / fa_mean,
                    # Variances
                    COVER_AREA_TOTAL_VAR = cover_var,
                    ...
                    # Sampling Errors
                    COVER_AREA_TOTAL_SE = sqrt(abs(cover_var)) / cover_mean * 100,
