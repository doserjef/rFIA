volumeStarter <- function(x, db, grpBy_quo = NULL, polys = NULL, 
  ...
  if (stringr::str_to_upper(volType) %in% c('NET', 'SOUND', 'GROSS') == FALSE) {
    warning(paste('volType', volType, 'unknown. Defaulting to net volume (NET).'))
  }
  ...
  # Select variables based on volType -------------------------------------
  # TODO: need to look through the volume types in the columns and stuff. 
  db$TREE <- db$TREE %>%
    dplyr::mutate(# Bole volume cubic feet
                  bcf = dplyr::case_when(stringr::str_to_upper(volType) == 'NET' ~ VOLCFNET,
                                         stringr::str_to_upper(volType) == 'GROSS' ~ VOLCFGRS, 
                                         stringr::str_to_upper(volType) == 'SOUND' ~ VOLCFSND), 
                  # Sawlog volume cubic feet
                  scf = dplyr::case_when(stringr::str_to_upper(volType) == 'NET' ~ VOLCSNET, 
                                         stringr::str_to_upper(volType) == 'GROSS' ~ VOLCSGRS,
                                         stringr::str_to_upper(volType) == 'SOUND' ~ VOLCSSND), 
                  # Sawlog volume board feet
                  sbf = dplyr::case_when(stringr::str_to_upper(volType) == 'NET' ~ VOLBFNET, 
  ...
  # TODO: need to go through handlePops
  ...
  # TODO: need to go through mergeSmallStrata
  ...
  db$TREE <- db$TREE %>%
    dplyr::select(PLT_CN, CONDID, DIA, SPCD, TPA_UNADJ, SUBP, TREE, 
                  dplyr::all_of(grpT), tD, typeD, bcf, scf, sbf) %>% 
  ...
  if (byPlot & !treeList) {
    ...
    if (treeList) {
      ...
      # TODO: need to go through sumToPlot
      ...
      # TODO: need to go through sumToEU
